...
bool HookInit()
{
	...
	for (DWORD i = 0; i < MAXN_PHOTO; i++)
	{
		...
		hCamouflageImage[i] = (HBITMAP)LoadImage(NULL, PhotoPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	}
...
{
	BOOL ret = trueEnumWindows(lpEnumFunc, lParam);

	char meg[1024];
	
	MessageBoxA(NULL, meg, "d", 0);
	if (bDisableProcessOperation) return FALSE;
